                                       ...
                                       int numiter, int ratio)
{
  ...
  void *request1 = malloc(200);
  free(request1);
  char *calloc_call = (char *)calloc(55, sizeof(char));
  int *int_calloc_call = (int *)calloc(55, sizeof(int));
  void *ptr_another = malloc(1024ULL * 1024ULL);
  free(calloc_call);
  free(int_calloc_call);
  free(ptr_another);
  ...
  for (i = 0; i < numiter; ++i)
  {
    ...
    else
    {
      ...
      free(data[index]);
      // printf("%p and index: %d, i: %d and numiter: %d and size: %ld\n", data[index], index, i, numiter, size);
      ++numfree;
      data[index] = 0;
    }
  }
